...
EO = arbol.find(17) # Estado objetivo
F = [EI]

def DFS(F:list[Node]): # Función de búsqueda - Depth First Search
    if F == []:
        print("F está vacía")
        return
    EA:Node = F.pop(0)
    if estado_Objetivo(EA):
        print(f"El estado actual \"{EA.data}\" es la meta")
        return
    OF = EA.children
    F = OF + F
...
DFS(F) # Llamamos a la función de búsqueda
print()